...
class SectorIndicesFlatList extends Component {

    _renderComponent = ({ item }) => {
        let sign = parseFloat(item.iislPercChange) > 0 ? 'gainers' : parseFloat(item.iislPercChange) == 0 ? 'loosers' : 'loosers';
        if(sign != this.props.check){
            return null;
        }
        return (
...
export default SectorIndicesFlatList;
